...
from helpers.api_helpers import get_category, get_category_from_api, get_category_from_db
...
class TestsCategoriesApi:
    ...
    def test_create_category(self, service, service_db):
        """Тест-кейс 04. Тестирование POST-запроса на создание рубрики"""
        # Создание рубрики с помощью POST-запроса
        new_category = get_category(TData.NEW_CATEGORY_DATA)
        api_response = service.create_category(new_category)
        ...
        # Рубрика из БД
        db_response = service_db.get_category(category_from_api.id)
        category_from_db = get_category_from_db(db_response)

        print()
        print(f"Category from api response = {category_from_api}")
        print(f"Category from DB = {category_from_db}")
    ...
    def test_update_category(self, service, service_db, category_id):
        """Тест-кейс 05. Тестирование POST-запроса на изменение рубрики"""
        # Рубрика из БД до изменения
        db_response_1 = service_db.get_category(category_id)
        category_from_db_1 = get_category_from_db(db_response_1)

        # Изменение рубрики с помощью POST-запроса
        api_response = service.update_category(category_id, TData.UPDATED_CATEGORY_DATA)
        ...
        db_response_2 = service_db.get_category(category_id)
        category_from_db_2 = get_category_from_db(db_response_2)

        print()
        print(f"Category from DB before update = {category_from_db_1}")
        print(f"Category from api response = {category_from_api}")
        print(f"Category from DB after update = {category_from_db_2}")
    ...
    def test_delete_category(self, service, service_db, category_id):
        ...
        api_response = service.delete_category(category_id, TData.DELETE_DATA)
        assert api_response.status_code == 200, "Статус-код ответа неверный"
        ...
        print()
        print(f"Category from DB before delete = {category_from_db_1}")
        print(f"Category from DB after delete = {category_from_db_2}")
