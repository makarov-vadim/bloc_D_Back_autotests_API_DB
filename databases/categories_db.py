...
from helpers.api_helpers import get_category_from_db
...
class CategoriesDB(WordPressDB):
    ...
    def get_category(self, term_taxonomy_id: int) -> CategoryModel | None:
        ...
        category_from_db = get_category_from_db(response)

        logging.info(f"Результат SQL-запроса на получение рубрики: {category_from_db}")
