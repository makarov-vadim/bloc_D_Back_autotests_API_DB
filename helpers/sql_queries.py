class SQLQueries:
    ...
    def get_columns_query(table_name):
        return f"SELECT GROUP_CONCAT(`COLUMN_NAME`) FROM `INFORMATION_SCHEMA`.`COLUMNS`WHERE `TABLE_SCHEMA` = 'wordpress' AND `TABLE_NAME` = '{table_name}';"

    @staticmethod
    def delete_query(table_name, obj_id, obj_id_name="id"):
        return f"DELETE FROM {table_name} WHERE {obj_id_name}={obj_id};"
    ...
    def delete_users_query():
        return f"DELETE FROM wp_users WHERE id <> 1;"

    @staticmethod
    def create_post_query(post_title, post_content, post_status):
        return ("INSERT INTO wp_posts ("
                        "post_author, "
                        "post_date, "
                        "post_date_gmt, "
                        "post_content, "
    ...
    def create_taxonomy_query(term_id, taxonomy):
        return ("INSERT INTO wp_term_taxonomy ("
                        "term_id, "
                        "taxonomy, "
                        "description"
                     ") "
    ...
    def get_term_query(term_id):
        return f"SELECT * FROM wp_terms WHERE term_id={term_id};"
